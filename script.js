...
class UltraFinancePro {
    ...
    formatCurrency(input) {
        if (input.value) {
            const value = parseFloat(input.value);
            if (!isNaN(value)) {
                input.style.color = value < 0 ? '#ff6b6b' : '#4facfe';
            }
        }
    }
    ...
    displayTransactions() {
        ...
        container.innerHTML = filteredTransactions.map(transaction => {
            ...
            const tags = transaction.tags && transaction.tags.length > 0 
                ? transaction.tags.map(tag => `<span style="background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; margin-right: 5px;">#${tag}</span>`).join('')
                : '';
            ...
            return `
                <div class="budget-item ${isExceeded ? 'exceeded' : ''}">
                    ...
                    <div class="budget-amounts">
                        ...
                        <div style="font-size: 0.8rem; color: ${remaining >= 0 ? '#4facfe' : '#fa709a'}">
